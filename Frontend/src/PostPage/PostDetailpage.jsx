...
const BlogDetailPage = () => {
    ...
    useEffect(() => {
        ...
        onValue(blogRef, (snapshot) => {
            ...
            else setBlog({ title: "Blog not found", content: "" });
        });
    ...
    return (
        <>
            ...
            <div className="max-w-4xl mx-auto my-12 p-6 bg-white rounded-lg shadow-md">
                ...
                <p className="text-gray-600 mb-6">By {blog.authorName || "Unknown"}</p>
                <p className="text-gray-700 whitespace-pre-line">{blog.content}</p>

                <Link to="/posts" className="inline-block mt-6 text-blue-600 underline">← Back to Blogs</Link>

                <div className="mt-10 border-t pt-6">
                    <h2 className="text-2xl font-bold mb-4">Comments</h2>
                    <form onSubmit={handleCommentSubmit} className="flex flex-col space-y-3">
                        <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={3} className="border p-3 rounded" placeholder="Add a comment..." />
                        <button type="submit" className="bg-black text-white p-2 rounded">Post Comment</button>
                    </form>

                    <div className="mt-6 space-y-4">
                        {comments.length > 0 ? comments.map((c, i) => (
                            <div key={i} className="border-b pb-3">
                                <Link to={`/profile/${c.userId}`} className="font-semibold text-blue-400 underline">{c.userName}</Link>
